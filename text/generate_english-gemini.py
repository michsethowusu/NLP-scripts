#!/usr/bin/env python3
"""
Automated English Text Generator using Gemini API
==============================================
Generates English text from provided paragraphs using Gemini API with async processing.
Includes validation, infinite retry logic (never skips rows), and resume support.

Usage:
    python generate_english.py --end-index 200000
    python generate_english.py AIza... --end-index 200000
    python generate_english.py --end-index 200000 --workers 20 --rpm 120
...
MAX_CONCURRENT    = 2          # parallel in-flight requests
REQUESTS_PER_MINUTE = 2        # target RPM (set to ~80% of your actual quota)
...
# ─── Prompts ──────────────────────────────────────────────────────────────────
PROMPT_TEMPLATE_1 = """Generate English text in 4 parts (each approximately 500 words):
1. Monologue - passionate, personal perspective
2. Narrative - journalistic, structured account  
3. Dialogue - conversational debate between 2 people
4. Storyful - dramatic, metaphorical storytelling

Use this text as inspiration:
{paragraph}

Guidelines:
- Write in natural, flowing English
- Each part should be substantial (~500 words)
- Return ONLY the English text, no preamble, no markdown formatting, no section headers"""

PROMPT_TEMPLATE_2 = """Create comprehensive English text with 4 distinct sections (each ~500 words total):
...
- Return plain English text only, no formatting or labels"""
...
def validate_text(text):
    """Validate English text. Returns (cleaned, char_count, reps_removed, is_valid, error)."""
...
class GeminiGenerator:
    def __init__(self, api_key, output_file="english_texts.jsonl", progress_file="progress.json"):
    ...
    async def _generate_forever(self, index: int, paragraph: str) -> dict:
        """
        Retries indefinitely until a valid English text is produced.
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate English text using Gemini API (never skips rows)",
    ...
    parser.add_argument("--end-index", type=int, default=100,
                        help="End index in dataset (default: 100)")
    ...
    parser.add_argument("--output", default="english_texts.jsonl",
                        help="Output JSONL file (default: english_texts.jsonl)")
